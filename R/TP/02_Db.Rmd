...
# Use absolute path while this issue is not fixed: https://github.com/gstlearn/gstlearn/issues/570
dat = Db_createFromNF(normalizePath(fileNF))
dat
```
...
*Db* objects (as all objects that inherits from *AStringable*) have a method `display` allowing to print a summary of the content of the data base. The same occurs when typing the name of the variable at the end of a chunk (see above).
...
Note: Non available values (NaN) are display in gray. This will be tunable in future versions.
...
# Use absolute path while this issue is not fixed: https://github.com/gstlearn/gstlearn/issues/570
grid = DbGrid_createFromNF(normalizePath(fileNF))
grid
```
...
p = plot.init(asp=1)
p = p + plot.raster(grid, name="Elevation", flagLegend=TRUE, legendName="Elevation")
p = p + plot.symbol(dat, nameSize="January_temp", flagLegend=TRUE, legendNameSize="Temperature", 
                    sizeRange=c(1,3), color="yellow")
