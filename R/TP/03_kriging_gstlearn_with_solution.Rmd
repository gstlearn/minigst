...
# Create an experimental omnidirectional variogram (with 20 lags separated by a distance of 100 m) for the log concentration of Cadmium using the Db obsDb (-> `vario_exp`)
...
```{r}
## Cross-validation for Ordinary kriging with moving neighborhood (Use the prefix "Xvalid_OK_mov")
...
```{r}
## Scatter plot Ordinary Kriging unique neighborhood VS moving neighborhood: Predictions
...
12. Perform an (ordinary) co-kriging prediction of the metal log-concentrations on the grid `targetDb` using the model `model_aniso4var`. Store the results using the prefix "COK".
